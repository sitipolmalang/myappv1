defmodule Myappv1Web.TagLive.Form do
  ...
  defp apply_action(socket, :edit, %{"id" => id}) do
    tag = Radio.get_tag!(id)

    socket
    |> assign(:page_title, "Edit Tag")
    |> assign(:tag, tag)
    |> assign(:form, to_form(Radio.change_tag(tag)))
  end
  ...
  def handle_event("validate", %{"tag" => tag_params}, socket) do
    changeset = Radio.change_tag(socket.assigns.tag, tag_params)
  ...
  defp save_tag(socket, :edit, tag_params) do
    case Radio.update_tag(socket.assigns.tag, tag_params) do
      {:ok, tag} ->
        {:noreply,
         socket
  ...
  defp save_tag(socket, :new, tag_params) do
    case Radio.create_tag(tag_params) do
      {:ok, tag} ->
        {:noreply,
         socket
